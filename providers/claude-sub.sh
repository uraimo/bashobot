#!/bin/bash
#
# Bashobot LLM Provider: Anthropic Claude (Subscription OAuth)
#
# Required auth: OAuth via ./bashobot.sh -login claude-sub
#

CLAUDE_SUB_MODEL="${CLAUDE_SUB_MODEL:-claude-sonnet-4-20250514}"
CLAUDE_SUB_API_URL="https://api.anthropic.com/v1/messages"

# Maximum tool call iterations to prevent infinite loops
MAX_TOOL_ITERATIONS=10

_claude_sub_get_token() {
    local creds
    if ! creds=$(oauth_get_access_token "claude-sub"); then
        echo "Error: Not logged in. Run ./bashobot.sh -login claude-sub" >&2
        return 1
    fi

    echo "$creds" | jq -r '.access // empty'
}

models_list() {
    echo "Claude subscription models use the same IDs as Anthropic (example: claude-sonnet-4-20250514)."
}

_claude_sub_build_request() {
    local messages="$1"
    local tools="$2"
    ...
    local system
    system=$(jq -n --arg system "$system_prompt" '[{"type":"text","text":"You are Claude Code, Anthropic\u0027s official CLI for Claude."},{"type":"text","text":$system}]')
...
_claude_sub_send_request() {
    local request_body="$1"
    local token="$2"

    local response
    response=$(curl -s -X POST "$CLAUDE_SUB_API_URL" \
...
_claude_sub_api_call() {
    local messages="$1"
    local tools="$2"

    local token
    token=$(_claude_sub_get_token) || return 1

    local request_body
    request_body=$(_claude_sub_build_request "$messages" "$tools")
    _claude_sub_send_request "$request_body" "$token"
}

_claude_sub_pack_response() {
    local text="$1"
    local request="$2"
    local response="$3"
...
llm_chat() {
    ...
    while [[ $iteration -lt $MAX_TOOL_ITERATIONS ]]; do
        ...
        api_result=$(_claude_sub_api_call "$current_messages" "$tools") || return 1
        ...
        if [[ -n "$error" ]]; then
            log_error "Claude subscription API error: $error"
            _claude_sub_pack_response "Error: $error" "$api_request" "$api_response"
            return 1
        fi
        ...
        if [[ "$stop_reason" == "tool_use" ]]; then
            ...
            for ((i=0; i<num_tools; i++)); do
                ...
                if [[ "$approval_required" == "true" ]]; then
                    ...
                    _claude_sub_pack_response "$prompt" "$api_request" "$api_response"
                    return 0
                fi
        ...
        if [[ -z "$text" ]]; then
            log_error "No text in Claude subscription response: $response"
            _claude_sub_pack_response "Sorry, I received an empty response." "$api_request" "$api_response"
            return 1
        fi

        _claude_sub_pack_response "$text" "$api_request" "$api_response"
        return 0
    done

    log_error "Max tool iterations reached"
    _claude_sub_pack_response "Sorry, I made too many tool calls. Please try a simpler request." "$api_request" "$api_response"
    return 1
}

# Provider info
llm_info() {
    echo "Provider: Claude (Subscription)"
    echo "Model: $CLAUDE_SUB_MODEL"
